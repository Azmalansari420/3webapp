    ...
    { 
        ...
        if (!empty($referal_code)) 
        {
            ...
            if (!$check_user_referal) {
                echo json_encode(["status" => 400, "message" => "Wrong Invitation Code.", "data" => []]);
                return;
            } else {
        ...
        if(empty($user))
        {
            ...
            {
                $row = $this->db->update("user_temp_register",$insertdata,array("mobile"=>$mobile,));
            }

            if($row)
            {
                $url = base_url('app/user/otp-confirm.php?device_id='.$device_id.'&token='.$firebase_token.'&mobile='.$mobile);
