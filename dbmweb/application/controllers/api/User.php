...
class User extends CI_Controller 
{
    ...
    /*update passweoerf*/
    public function payment_password_update()
    {
        ...
        if(!empty($user))
        {
            ...
            if($user->payment_password == $npassword) 
            {
                $result['status'] = "400";
                $result['message'] = "New password cannot be the same as old password.";
                $result['data'] = array();
    ...
    /*upadte bank details*/
    public function bakdetails()
    {
        
        $user_id = $this->session->userdata("user")['id']; 

        $bank_username = $this->input->post("bank_username");
