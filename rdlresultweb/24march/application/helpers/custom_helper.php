...
{
  $ci =& get_instance();
  $category = $ci->crud->selectDataByMultipleWhere('all_times',array('id'=>$value));
  if(!empty($category))
  {
    $name = date('h:i A',strtotime($category[0]->time));
  }
  else
  {
    $name = 'Not Found';
...
function gametimein24hour($value)
{
  $ci =& get_instance();
  $category = $ci->crud->selectDataByMultipleWhere('all_times',array('id'=>$value));
  if(!empty($category))
  {
    $name = $category[0]->time;
  }
  else
  {
    $name = 'Not Found';
