               ...
               <div class="card ">
                  <div class="card-header card-header-inverse">
                     ...
                     <div class="card-header-btn">
                        ...
                        <form > 
                            <select name="game_id" class="form-sel">
                            ...
                            <input type="date" name="selectdate" class="form-sel">
                            <button id="search" class="btn btn-purple">Submit</button>
                         </form>
                     </div>
                  </div>
...
/*--pagination--*/
   ...
   function loadTableData(offset = 0) 
   {
        ...
        $.ajax({
            type: 'POST',
            url: '<?=base_url($pagination_url) ?>',
            data: { game_id: game_id, selectdate: selectdate, limit: 10, offset: offset },
            success: function(data) {
                var jsonData = JSON.parse(data);
                $('#table-data').html(jsonData.html);
  ...
  $(document).ready(function() {
      $('#search').on('click', function(event) {
        ...
        $.ajax({
          type: 'POST',
          url: '<?=base_url("admin_con/number/get_table_data") ?>', // Make sure the URL is correct
          data: { game_id: game_id, selectdate: selectdate, limit: 10, offset: 0 },
          success: function(data) {
            var jsonData = JSON.parse(data);
            $('#table-data').html(jsonData.html);
            $('#pagination-links').html(jsonData.pagination_links);
