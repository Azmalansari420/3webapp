        ...
        <div class="mt-5 tf-tab">
          <div class="tf-tab box-components mt-0 tf-container" style="paddi1ng1: 10px 0;">
            ...
            <table class="table"> 
              ...
              <tbody>
                ...
                $this->db->limit(5);
                $topPerformers = $this->db->get()->result();

                $excludedAsmIds = []; // To track asm_id
                foreach($topPerformers as $data) { 
                    $i++; 
                    $name = userdetails($data->asm_id);
                    $totalSpent = (float) $data->total_spent;
                    $chartData[] = [$name, $totalSpent];
                    $excludedAsmIds[] = $data->asm_id; // Track asm_id
                    ?>
                    <tr>
                        <td><?= $name ?></td>
                        <td><?= $i ?></td>
          ...
          <table class="table"> 
              ...
              <tbody>
                ...
                // Fetch Lowest Performers excluding top performers
                $this->db->select('rsm_id, sales_office_id, asm_id, city, state, user_id, SUM(after_discount_final_price) as total_spent');
                $this->db->from('finalorder');
                $this->db->where('nsm_id', $full_detail->id);
                if (!empty($excludedAsmIds)) {
                    $this->db->where_not_in('asm_id', $excludedAsmIds); // Exclude top performers
                }
                $this->db->group_by('asm_id');
                $this->db->order_by('total_spent', 'ASC');
                $this->db->limit(5);
                $lowestPerformers = $this->db->get()->result();

                foreach($lowestPerformers as $data) { 
                    $i++; 
                    $name = userdetails($data->asm_id);
                    $totalSpent = (float) $data->total_spent;
                    $chartData[] = [$name, $totalSpent];
