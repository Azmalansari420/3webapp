      ...
      <div class="card-secton" style="margin-top: 0;">
        ...
        <div class="mt-5 tf-tab">
          <div class="tf-tab box-components mt-0 tf-container" style="paddi1ng1: 10px 0;">
            <h3 class="fw_6 d-flex justify-content-between mb-3"> Top 5 RSM Performance </h3>
            ...
            <table class="table"> 
                ...
                <tbody>
                  ...
                  $i=0;
                  $this->db->select('rsm_id,city,state, SUM(after_discount_final_price) as total_spent');
                  $this->db->from('finalorder');
                  $this->db->where('nsm_id', $full_detail->id);
                  $this->db->group_by('rsm_id');
                  ...
                  foreach($userorder as $data)
                    ...
                    <tr>
                      <td><?=$i ?></td>
                      <td><?=usersdetails($data->rsm_id) ?><!-- (<?=walletamt($data->rsm_id) ?>) --> </td>
                      <td><?=statename(user_statename($data->rsm_id)) ?></td>
                      <td><?=cityname(user_city($data->rsm_id)) ?></td>
              ...
              <?php
                  ...
                  $i=0;
                  $this->db->select('sales_office_id,rsm_id,asm_id,city,state,user_id, SUM(after_discount_final_price) as total_spent');
                  $this->db->from('finalorder');
                  $this->db->where('nsm_id', $full_detail->id);
                  $this->db->group_by('rsm_id');
                  $this->db->order_by('total_spent', 'DESC');
