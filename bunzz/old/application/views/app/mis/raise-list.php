                        ...
                        <div class="tab-pane fade show active" id="tabIcon1" role="tabpanel">
                            ...
                            <ul class="mt-3 mb-5">
                              <?php
                                ...
                                $users = $this->crud->selectDataByMultipleWhere('raise_asm_to_rsm', array('rsm_id' => $full_detail->id,));
                                    ...
                                    <li class="list-card-invoice">
                                        ...
                                        <div class="content-right">
                                            <h4>
                                                ...
                                                if($data->status==1)
                                                    { ?>
                                                <a href="<?= ('rsm/raise-add.php?id=' . $data->id) ?>">
                                                <?php } else {?>
                                                    <a href="<?= ('rsm/raise-add.php?id=' . $data->id) ?>">
                                                <?php } ?>
                                                    <?=raiseissue($data->issue_type) ?><br> [from <?=userdetails($data->asm_id) ?>]
...
<script>
    ...
    $(document).ready(function () {
        $('#downloadExcel').on('click', function () {
            $.ajax({
                url: '<?=base_url() ?>api/rsm/raiseexceldata',
                type: 'POST',
