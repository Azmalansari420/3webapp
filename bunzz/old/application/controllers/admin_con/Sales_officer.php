...
class Sales_officer extends CI_Controller
{
	...
	protected $arr_values = array(
						   	'page_title'=>'Sales Officer',
						   	'table_name'=>'users',
						   	'upload_path'=>'media/uploads/distributor/',
						   	'load_path'=>'admin/sales_officer/',
						   	'redirect_path'=>'admin_con/sales_officer/',
						   	'back_url'=>'sales_officer',
						   	'add_url'=>'sales_officer',
						   	'edit_url'=>'sales_officer',
						   	'delete_url'=>'sales_officer',
						   	'view_url'=>'sales_officer',
						   	'status_value'=>'sales_officer',
						   	'multiple_delete'=>'admin_con/sales_officer/delete_all',
						   	'pagination_url'=>'admin_con/sales_officer/listing',
						   	'controller_name'=>'sales_officer',
						   	'page_name'=>'sales_officer.php',
						   ); 
   ...
   //--check user login or not
	...
	function add()
	{
		check_controller_inner_access(21,2);
		if(isset($_POST['submit']))
		{
			...
			$data['join_id'] = $this->session->userdata("id");	
			$data['user_id'] = generate_user_id();		
			$data['nsm_id'] = $this->input->post('nsm_id');	

			$data['rsm_id'] = $this->input->post('rsm_id');			
			$data['asm_id'] = $this->input->post('asm_id');			
			...
			{
				$this->session->set_flashdata('message','<div class="toast show" role="alert" aria-live="assertive" aria-atomic="true"><div class="toast-header"><div class="bg-blue rounded w-25px h-25px d-flex align-items-center justify-content-center text-white"><i class="fa fa-bell"></i></div><strong class="me-auto ms-2">Message</strong><button type="button" class="btn-close" data-bs-dismiss="toast" aria-label="Close"></button></div><div class="toast-body" style="color:black;">Email Already Register</div></div>');
	...
	function listing()
	{
		check_controller_inner_access(21,1);	
		$search = $this->input->post("search");

		$this->load->library("pagination");			
		$config = array();  
    $config["base_url"] = base_url($this->arr_values['redirect_path'].'listing');
    $config["total_rows"]  = $totalrow = count($this->crud->selectDataByMultipleWhere($this->arr_values['table_name'],array('role'=>4)));
    ...
    if(!empty($search))
    {
    	$this->db->where(" concat(name,mobile,address) like '%$search%' ");
	...
	public function delete($id)
	  ...
	  function delete_all()
		{
			check_controller_inner_access(21,4);	
	...
	function edit()
	{
		check_controller_inner_access(21,3);		 
		$args=func_get_args();
...
$data['nsm_id'] = $this->input->post('nsm_id');	
	...
	function view()
	{
		 check_controller_inner_access(21,5);
		$args=func_get_args();
		$data['page_title'] = $this->arr_values['page_title'];
		$data['upload_path'] = $this->arr_values['upload_path'];
		$data['back_url'] = base_url('admin_con/'.$this->arr_values['back_url'].'/listing');
...
//---------------------status
	...
	function showasm_list()
	{
		$result = array();
		$html = '<option Selected>Select ASM</option>';
		$id = $this->input->post('id');
	...
	{
		$result = array();
		$html = '<option Selected>Select Sales Officer</option>';
		$id = $this->input->post('id');
