...
class Asm extends CI_Controller
{
	...
	protected $arr_values = array(
						   	...
						   	'view_url'=>'asm',
						   	'status_value'=>'asm',
						   	'multiple_delete'=>'admin_con/asm/delete_all',
						   	'pagination_url'=>'admin_con/asm/listing',
						   	'controller_name'=>'asm',
						   	'page_name'=>'asm.php',
						   ); 
   ...
   //--check user login or not
	...
	function add()
	{
		check_controller_inner_access(20,2);
	...
	function listing()
	{
		check_controller_inner_access(20,1);	
		$search = $this->input->post("search");

		$this->load->library("pagination");			
		$config = array();  
    $config["base_url"] = base_url($this->arr_values['redirect_path'].'listing');
    $config["total_rows"]  = $totalrow = count($this->crud->selectDataByMultipleWhere($this->arr_values['table_name'],array('role'=>3)));
	...
	public function delete($id)
	  ...
	  function delete_all()
		{
			check_controller_inner_access(20,4);	
	...
	function edit()
	{
		check_controller_inner_access(20,3);		 
		$args=func_get_args();
...
$data['nsm_id'] = $this->input->post('nsm_id');
	...
	function view()
	{
		 check_controller_inner_access(20,5);
		$args=func_get_args();
		$data['page_title'] = $this->arr_values['page_title'];
		$data['upload_path'] = $this->arr_values['upload_path'];
		$data['back_url'] = base_url('admin_con/'.$this->arr_values['back_url'].'/listing');
...
//---------------------status
	...
	{
		$result = array();
		$html = '<option Selected>Select RSM</option>';
		$id = $this->input->post('id');
		$city = $this->crud->selectDataByMultipleWhere('users',array('nsm_id'=>$id,'role'=>2));
		if(!empty($city))
		{
