      ...
      <div class="card-secton" style="margin-top: 0;">
        ...
        <div class="mt-5 tf-tab">
          <div class="tf-tab box-components mt-0 tf-container" style="paddi1ng1: 10px 0;">
              ...
              $walletamount = 0;
               $chartData = [['ASM Name', 'Wallet Amount']]; 
              // $this->db->group_by('asm_id');
              $asm_users = $this->db->get_where('users',array('role'=>3))->result_object();

         

              foreach($asm_users as $data)
              { 


                // $this->db->group_by('asm_id');
                $distributer = $this->db->get_where('users',array('asm_id'=>$data->id,'role'=>5))->result_object();
                   foreach ($distributer as $value) 
                   {
                    $walletamount += $value->wallet;
                      // $walletamount = $value->wallet;
                      // echo '<h2>'.$walletamount.'</h2>';
                      // echo '<h2>'.$data->name.'</h2>';
                   } 
                   // echo '<h2>ASM Name: '.$data->name.' - Total Wallet Amount: '.$walletamount.'</h2>';

                   $total_wallet_amount += $walletamount;
                   $chartData[] = [$data->name . ' - ' . currency_simble($walletamount), $walletamount]; 
                ...
                ?>
              <div class="fw_6 d-flex justify-content-around mb-3">
                <h3 style="    font-size: 15px;">Total Wallet Amount:- <?=currency_simble($total_wallet_amount) ?></h3>
              <!-- <h3 > ASM </h3> -->
              </div>
                <h3 class="fw_6 d-flex justify-content-between mb-3"> Wallet money of ASM</h3>
              ...
              <table class="table"> 
                ...
                <tbody >
                  <?php
                    $total_wallet_amount2 = 0;
                    $wallet_data = []; // Array to hold wallet amounts and corresponding ASM details

                    // Fetch ASM users
                    $asm_users2 = $this->db->get_where('users', array('rsm_id' => $full_detail->id, 'role' => 3))->result_object();

                    foreach ($asm_users2 as $data) {
                        $walletamount2 = 0; // Reset wallet amount for each ASM

                        // Get all distributors for the current ASM
                        $distributer2 = $this->db->get_where('users', array('asm_id' => $data->id, 'role' => 5))->result_object();
                        
                        foreach ($distributer2 as $value) {
                            $walletamount2 += $value->wallet; 
                        }

                        // Store the wallet amount and ASM details in an array
                        $wallet_data[] = [
                            'asm_id' => $data->id,
                            'asm_name' => userdetails($data->id),
                            'distributers_count' => count($distributer2), 
                            'wallet_amount' => $walletamount2
                        ];
                    }

                    // Sort the wallet_data array by wallet_amount in descending order
                    usort($wallet_data, function($a, $b) {
                        return $b['wallet_amount'] <=> $a['wallet_amount'];
                    });

                    // Output the table
                    foreach ($wallet_data as $entry) {
                        ?>
                        <tr>
                            <td><?= $entry['asm_name'] ?></td>
                            <td><?= $entry['distributers_count'] ?></td>
                            <td><?= currency_simble($entry['wallet_amount']) ?></td>
                        </tr>
                        <?php
                    }
                    ?>
              ...
              <div id="piechart_3d" style="width: 350px; height: 100%;"></div>
                  ...
                  <li class="nav-item col-6" style="display:none;">
                      ...
                      <select class="form-select sales-officer-list" name="sales_office_id" >
                        <option selected value="">Select Sales Officer</option>
                         <?php
                         $rsm = $this->crud->selectDataByMultipleWhere('users',array('role'=>4,'status'=>1,));
                         foreach($rsm as $data)
                            { ?>
                         <option value="<?=$data->id ?>" ><?=$data->name ?></option>
                      <?php } ?>
                  ...
                  <li class="nav-item col-6" >
                      ...
                      <select class="form-select distributer-list" name="distributer_id" >
                        <option selected value="">Select Distributer</option>
                         <?php
                         $rsm = $this->crud->selectDataByMultipleWhere('users',array('role'=>5,'status'=>1,));
                         foreach($rsm as $data)
                            { ?>
                         <option value="<?=$data->id ?>" ><?=$data->name ?></option>
                      <?php } ?>
...
function getdata() {
    ...
    var settings = {
        url: "<?=base_url() ?>api/mis/leadgerdata",
        method: "POST",
        dataType: "json",
        processData: false,
        contentType: false,
