      ...
      <div class="card-secton" style="margin-top: 0;">
                ...
                <div class="wallet-footer">
                    <ul class="d-flex justify-content-between align-items-center">
                        <li class="wallet-card-item">
                            <a href="add-distributor.php" class="fw_6 text-center" id="btn-popup-down">
                                <ul class="icon icon-user3">
                                    <li class="path1"></li>
                                    <li class="path2"></li>
                                    <li class="path3"></li>
                                </ul>
                        ...
                        <li class="wallet-card-item"><a class="fw_6" href="search-filter.php">
                                <ul class="icon icon-search">
                                    <li class="path1"></li>
                                    <li class="path2"></li>
                                    <li class="path3"></li>
                                    <li class="path4"></li>
                        ...
                        <li class="wallet-card-item"><a class="fw_6 btn-card-popup" href="distributer-active-list.php">
                                <ul class="icon icon-calendar3">
                                    <li class="path1"></li>
                                    <li class="path2"></li>
                                    <li class="path3"></li>
                                </ul>
        ...
        <div class="mt-5 tf-tab">
          <div class="tf-tab box-components mt-0 tf-container" style="paddi1ng1: 10px 0;">
            <h3 class="fw_6 d-flex justify-content-between mb-3"> Top 5 Distributers Performance </h3>

            <table class="table"> 
                <thead>
                    <tr>
                        <th>Ranking</th>
                        <th>Distributer Firm Name</th>
                        <th>State</th>
                        <th>City</th>
                    </tr>
                </thead>
                <tbody >
                  <?php
                  $i=0;
                  $this->db->select('sales_office_id,user_id,asm_id,city,state, SUM(after_discount_final_price) as total_spent');
                  $this->db->from('finalorder');
                  $this->db->where('sales_office_id', $full_detail->id);
                  ...
                  foreach($userorder as $data)
                    ...
                    <tr>
                      ...
                      <td><?=statename(distributerfirmstate($data->user_id)) ?></td>
                      <td><?=cityname(distributerfirmcity($data->user_id)) ?></td>                    
                    </tr>
                  <?php } ?>
                </tbody>
              </table>
              ...
              <?php
              if ($full_detail && isset($full_detail->id)) {
                  // First query to count total sales officers
                  $this->db->select('COUNT(user_id) as total_sales_officers');
                  $this->db->from('finalorder');
                  $this->db->where('sales_office_id', $full_detail->id);
                  $this->db->group_by('user_id');
                  $result = $this->db->get()->row();
                  
                  // Check if the result is not null before accessing total_sales_officers
                  $total_sales_officers = $result ? $result->total_sales_officers : 0; 
                  ...
                  foreach($userorder as $data) { 
                      $i++; 
                      $name = userdetails($data->user_id); 
                      $totalSpent = (float) $data->total_spent; 
                      $chartData[] = [$name, $totalSpent];
                  }
              } else {
                  // Handle the case where $full_detail is null
                  echo "No details available for this sales office.";
              }
              ?>
        ...
        <div class="mt-5 tf-tab" style="margin-bottom: 50px;">
          <div class="tf-tab box-components mt-0 tf-container" style="paddi1ng1: 10px 0;">
            ...
            <h3 class="fw_6 d-flex justify-content-between mb-3"> Download Report </h3>
                  ...
                  <li class="nav-item col-6" >
                      ...
                      <select class="form-select distributer-list" name="distributer_id" >
                        <option selected value="">Select Distributer</option>
                         <?php
                         $rsm = $this->crud->selectDataByMultipleWhere('users',array('role'=>5,'status'=>1,'sales_office_id'=>$full_detail->id));
                         foreach($rsm as $data)
                            { ?>
              ...
              <div class="tab-content mt-4">
                <!-- <div class="highest-amount"></div> -->
                  <div class="tab-pane fade show active" id="item1" role="tabpanel" style="display:none;">
                      <div style="max-height: 500px; max-width: 100%; overflow-x: auto; overflow-y: auto;">
        ...
        <div class="bill-content">
          <div class="container">
              <h3 class="fw_6 d-flex justify-content-between mt-3">Approved Distributor List <a href="distributer-active-list.php">View All</a></h3>
              <ul class="mt-3 mb-5">
                <?php
                $this->db->limit(6);
                $this->db->order_by('id desc');
                $users = $this->crud->selectDataByMultipleWhere('kyc_records',array('sales_office_id'=>$full_detail->id,'status'=>1));
                foreach($users as $data)
                  ...
                  <li class="list-card-invoice">
                      ...
                      <div class="content-right">
                          <h4>
                            <a style="font-weight: 700;" href="<?=('rsm/distributor-kyc-details.php?id='.$data->id) ?>"><?=$data->firm_name ?> 
                              <?=distributer_app_status($data->status) ?>
                            </a>
                          </h4>
                          <p style="font-weight: 700;color: black;"><?=$data->name ?> <br> <?=$data->mobile ?></p>
                      </div>
                  </li>
        ...
        <!-- <div class="mt-5 tf-tab">
          <div class="tf-tab box-components mt-0 tf-container" style="paddi1ng1: 10px 0;">
            ...
            <h3 class="fw_6 d-flex justify-content-between mb-3"> Filter </h3>
              ...
              <form  method="get" class="nav nav-tabs lined row" role="tablist" style="justify-content: space-1around!important;">
                  ...
                  <li class="nav-item col-6" >
                      ...
                      <select class="form-select distributer_id" name="distributer_id" >
...
<script type="text/javascript">
  ...
  function drawChart() {
      ...
      var options = {
          title: 'Total Spent By Distributer',
          is3D: true,
      };
...
//     var settings = {
//         "url": "<?=base_url() ?>api/Sales_officer/leadgerdata",
//         "method": "POST",
//         "dataType": "json",
//         "timeout": 0,
//         "processData": false,
...
//                 dataPoints.push({ label: value.user_id, y: value.total_spent });
//             });
...
//             var dataPoints2 = [['User', 'Total Spent']]; // Google Charts expects this format
//             response.data.forEach(function(value) {
//                 dataPoints2.push([value.user_id, value.total_spent]);
//             });

//             // Create and render the pie chart
//             google.charts.load('current', {packages:['corechart']});
//             google.charts.setOnLoadCallback(function() {
//                 var data = google.visualization.arrayToDataTable(dataPoints2);
//                 var options = {
//                     title: 'Total Spent by Distributer',
//                     is3D: true,
