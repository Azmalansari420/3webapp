...
class Mis extends CI_Controller 
{
     ...
     public function leadgerdata()
    {
        ...
        $sheet->setCellValue('A1', 'ASM EMP')
          ->setCellValue('B1', 'ASM Name')
          ->setCellValue('C1', 'ASM Contact')
          ->setCellValue('D1', 'SO EMP')
          ->setCellValue('E1', 'SO Name')
          ->setCellValue('F1', 'SO Contact')
          ->setCellValue('G1', 'DS Code')
          ->setCellValue('H1', 'DS Componey Name')
          ->setCellValue('I1', 'DS Name')
          ->setCellValue('J1', 'DS Contact')
          ->setCellValue('K1', 'State')
          ->setCellValue('L1', 'Distric')
          ->setCellValue('M1', 'Month 1')
          ->setCellValue('N1', 'Month 2')
          ->setCellValue('O1', 'Month 3')
          ->setCellValue('P1', 'Month 4')
          ->setCellValue('Q1', 'Total Sale');

        $row = 2;
        foreach ($userorder as $order) {
            $sheet->setCellValue('A' . $row, empcode($order->asm_id))
                  ->setCellValue('B' . $row, usersdetails($order->asm_id))
                  ->setCellValue('C' . $row, usermobile($order->asm_id))
                  ->setCellValue('D' . $row, empcode($order->sales_office_id))
                  ->setCellValue('E' . $row, usersdetails($order->sales_office_id))
                  ->setCellValue('F' . $row, usermobile($order->sales_office_id))
                  ->setCellValue('G' . $row, empcode($order->user_id))
                  ->setCellValue('H' . $row, distributerfirm($order->user_id))
                  ->setCellValue('I' . $row, usersdetails($order->user_id))
                  ->setCellValue('J' . $row, usermobile($order->sales_office_id))
                  ->setCellValue('K' . $row, statename($order->state))
                  ->setCellValue('L' . $row, cityname($order->city))
                  ->setCellValue('M' . $row, $order->month_1_spent)
                  ->setCellValue('N' . $row, $order->month_2_spent)
                  ->setCellValue('O' . $row, $order->month_3_spent)
                  ->setCellValue('P' . $row, $order->month_4_spent)
                  ->setCellValue('Q' . $row, $order->total_spent);
            $row++;
        }
...
/*--RSM sales officer list--*/
function showrsm_list()
    {
        $result = array();
        $html = '<option Selected value="">--Select--</option>';
        $id = $this->input->post('id');
        $city = $this->crud->selectDataByMultipleWhere('users',array('rsm_id'=>$id,'role'=>3));
        if(!empty($city))
        {
...
public function leadgerdata2()
    {
        ...
        $ledger_pdf_view = $this->load->view('app/rsm/ledger-pdf', ['userorder' => $userorder], true);
        ...
        $downloadUrl = base_url('api/rsm/download_ledger_pdf?from_date=' . $from_date . '&to_date=' . $to_date.'&user_id='.$user_id);
    ...
    public function download_ledger_pdf()
    {
        ...
        $html = $this->load->view('app/rsm/ledger-pdf', ['userorder' => $userorder], true);
    ...
    public function upadate_target()
    {
        ...
        {   
            ...
            if($status==2)
            {
                $url = base_url('app/rsm/assign-target.php?id='.$id.'&device_id='.$device_id.'&firebase_token='.$firebase_token);
            }
            else
            {
                $url = base_url('app/rsm/my-target.php?device_id='.$device_id.'&firebase_token='.$firebase_token);
            }
...
/*assign target*/
    public function assign_target()
        {
            ...
            if(!empty($insertdata))
            {
                $this->db->insert('target_rsm_to_asm',$insertdata);
                $url = base_url('app/rsm/assign-target-list.php?device_id='.$device_id.'&firebase_token='.$firebase_token);
    ...
    public function delete_target()
    {
        ...
        if(!empty($id))
        {   
            $this->db->delete('target_rsm_to_asm',array('id'=>$id));
            // die;
            $url = base_url('app/rsm/assign-target-list.php?device_id='.$device_id.'&firebase_token='.$firebase_token);
     ...
     public function upadate_notice()
    {
        ...
        {   
            ...
            $url = base_url('app/rsm/notice-list.php?device_id='.$device_id.'&firebase_token='.$firebase_token);
            $result['status']  = "200";            
            $result['data']  = $updatedata;
            $result['url']  = $url;
        }
     ...
     public function add_upadate_raise()
    {
        ...
        {   
            ...
            $url = base_url('app/rsm/raise-list.php?device_id='.$device_id.'&firebase_token='.$firebase_token);
            $result['status']  = "200";            
            $result['data']  = $updatedata;
            $result['url']  = $url;
        }
    ...
    public function raiseexceldata()
    {
        ...
        $sheet->setCellValue('A1', 'RSM NAME')
          ->setCellValue('B1', 'ASM Name')
          ->setCellValue('C1', 'Issue Type')
          ->setCellValue('D1', 'Message')
          ->setCellValue('E1', 'Reply')
          ->setCellValue('F1', 'Status')
          ->setCellValue('G1', 'Date');

        $row = 2;
        foreach ($userorder as $order) {
            $sheet->setCellValue('A' . $row, usersdetails($order->rsm_id))
                  ->setCellValue('B' . $row, usersdetails($order->asm_id))
                  ->setCellValue('C' . $row, raiseissue($order->issue_type))
                  ->setCellValue('D' . $row, $order->message)
                  ->setCellValue('E' . $row, $order->reply_message)
                  ->setCellValue('F' . $row, raiseexcel($order->status))
                  ->setCellValue('G' . $row, ($order->addeddate));
            $row++;
        }
    ...
    public function update_order_status()
    {
        ...
        {   
            ...
            $this->db->update('finalorder',array('order_status'=>$order_status,'reject_msg'=>$reject_msg));
