...
class Withdraw_request extends CI_Controller
{
	...
	protected $arr_values = array(
						   	...
						   	'pagination_url'=>'admin_con/withdraw_request/listing',
						   ); 
   ...
   //--check user login or not
    ...
    if(!empty($search))
    {
    	$this->db->where(" concat(upi_no) like '%$search%' ");
...
//---------------------status
	...
	public function status_change()
	{
		...
		{						
			...
			$data['reject_reason'] = $reject_reason = $this->input->post('reject_reason');		
			$this->db->update($this->arr_values['table_name'],$data,array("id"=>$id));

			$get_user_wallet = $this->db->get_where('users',array('id'=>$user_id))->result_object();
			...
			if($status==2)
			{
		      ...
		      $this->db->update("user_history",array("status"=>$status,'reject_reason'=>$reject_reason),array("join_id"=>$id,));   

			}
			else
			{
				$this->db->update("user_history",array("status"=>$status,),array("join_id"=>$id,));  
			}
	...
	function view_user_trans()
	{
		$data['page_title'] = $this->arr_values['page_title'];
		$data['upload_path'] = $this->arr_values['upload_path'];
		$data['back_url'] = base_url('admin_con/'.$this->arr_values['back_url'].'/listing');
		$this->load->view($this->arr_values['load_path'].'view-user-details',$data);
	}



	function view_user_game()
	{
		$data['page_title'] = $this->arr_values['page_title'];
		$data['upload_path'] = $this->arr_values['upload_path'];
		$data['back_url'] = base_url('admin_con/'.$this->arr_values['back_url'].'/listing');
		$this->load->view($this->arr_values['load_path'].'user-game',$data);
	}
