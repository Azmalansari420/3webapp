<?php 
	...
	if(isset($form_submit)){
		...
		$nextxxx2 = fs("game_times",array("id"=>$game_time_id,));


$nextxxx = fs("all_times",array("id"=>$nextxxx2->game_time_id,));
		...
		if($form_action==2){
			//update code
			$where = array('id'=>$c_id);
		 	$row   = $con->update('number',$col_val,$where);
			
		    //alert message
			if($row==1){
				$reload_path = "add_number.php?id=".$c_id;
			...
			//alert message
			if($row==1){
				alert('Successfully Added','add_number.php');
			}
...
?>
 ...
 Back</a></span>
				...
				<div class="col-sm-4 col-xs-6">			
					<label for="status">Select Game :</label>
                        <select required class="form-control" name="game_id" id="game_id">
				...
				foreach($fetch_all_game as $fag){ 
					...
					<div class="col-sm-4 col-xs-6">			
					<label for="status">Select Game time :</label>
                        <select required class="form-control" name="game_time_id" id="time12222">
